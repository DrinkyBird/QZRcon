    ...
    rcon(rcon)
{
    ...
    connect(rcon, SIGNAL(completions(QStringList, int)), this, SLOT(onCompletionReceived(QStringList, int)));
    connect(rcon, SIGNAL(tooManyCompletions(int)), this, SLOT(onTooManyCompletions(int)));
    ...
    ui->lineCommand->installEventFilter(this);
}
...
void RconWindow::loadConfig()
{
    ...
    QFont font;
#ifdef WIN32
        font.fromString(settings->value("font", QFont("Lucida Console", 10).toString()).toString());
#else
        font.fromString(settings->value("font", QFont("Courier", 10).toString()).toString());
#endif
        ui->log->setFont(font);
    settings->endGroup();
}
...
void RconWindow::log(QString string, bool resetCursor)
{
    if(resetCursor)
        ui->log->moveCursor(QTextCursor::End);

    ui->log->insertHtml(string);
    ui->log->insertHtml("<br>");

    if(resetCursor)
        ui->log->moveCursor(QTextCursor::End);
}

void RconWindow::onStateChange(int state)
{
...
void RconWindow::onCompletionReceived(QStringList commands, int amount)
{
    if (amount == 0)
    {
        log("<b>There are no auto-completions.");
    }
    else
    {
        QString toBox = QString("<b>There %1 %2 completion%3:</b><br>")
                        .arg((amount == 1) ? "is" : "are")
                        .arg(amount)
                        .arg((amount == 1) ? "" : "s");
        ...
        {
            QString toAppend = QString("%1%2")
                               .arg(commands[i])
                               .arg((i == amount - 1) ? "" : ", ");
            toBox.append(toAppend);
        }

        if(amount == 1)
            ui->lineCommand->setText(commands[0]);

        log(toBox);
    }
}

void RconWindow::onTooManyCompletions(int amount)
{
    QString toBox = QString("There are too many auto-completions.\nTry and narrow it down.");
...
void RconWindow::send()
{
    ...
    if(!ui->checkChat->isChecked())
        log(QString("<b><font color='#0F0'>&gt;</font></b> <font color='#2E7D32'>%1</font>").arg(command));
...
// Tab completion.
bool RconWindow::eventFilter(QObject *obj, QEvent *event)
{
  if (obj == ui->lineCommand) {
     if (event->type() == QEvent::KeyPress) {
         QKeyEvent *keyEvent = static_cast<QKeyEvent*>(event);
         if (keyEvent->key() == Qt::Key_Tab)
         {
             if (ui->checkChat->isChecked())
                 log("<b><font color='#F00'>Can not use auto-complete in chat mode.</font></b>");
             else
             {
                 QString acInput = ui->lineCommand->text();
                 qDebug() << "Fetching auto-completions for " << acInput << endl;
                 rcon->tabComplete(acInput);
             }

...
/* QT WRAPPER */
// FIXME: 4 kilobytes max.
QByteArray huffmanEncode(QByteArray data)
{
    const unsigned char *in = (const unsigned char*)data.constData();
    unsigned char out[4096];
    int outlen;

    HUFFMAN_Encode(in, out, data.size(), &outlen);
    return QByteArray((char*)out, outlen);
}
QByteArray huffmanDecode(QByteArray data)
{
    ...
    HUFFMAN_Decode(in, out, data.size(), &outlen);
    return QByteArray((char*)out, outlen);
}
...
static int						huffman_GetBit( const unsigned char *puf, int pos );
...
int huffman_GetBit(const unsigned char *buf,int pos)
{
